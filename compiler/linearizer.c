...
int linearizeStatementList(int currentTACIndex, struct LinkedList *blockList, struct BasicBlock *currentBlock, struct BasicBlock *controlConvergesTo, struct astNode *it, int *tempNum, int *labelCount, struct tempList *tl)
{
    ...
    while (runner != NULL)
    {
        ...
        case t_if:
        {
            ...
            struct Stack *beforeConvergeExpirations = Stack_new();
            ...
            while (resultBlocks->size > 0)
            {
                ...
                for (struct LinkedListNode *TACRunner = poppedBlock->TACList->tail; TACRunner != NULL; TACRunner = TACRunner->prev)
                {
                    ...
                    if (examinedTAC->operation == tt_expireatinterval)
                    {
                        Stack_push(beforeConvergeExpirations, examinedTAC);
                        break;
                    }
                }
    ...
    {
        // make a line to expire variables at the interval jumped to when exiting the block
        struct TACLine *beforeRestoreExpire = newTACLine(currentTACIndex, tt_expireatinterval);
        BasicBlock_append(currentBlock, beforeRestoreExpire);
